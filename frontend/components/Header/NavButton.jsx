...
const NavButton = ({ onClick, icon, label, subtext, colorClass = "bg-white/5", iconColor = "text-white" }) => {
  return (
    <div className="group relative flex items-center justify-center">
      {/* THE BUTTON */}
      <button 
        onClick={onClick}
        className={`flex items-center justify-center w-9 h-9 ${colorClass} hover:brightness-110 border border-white/10 rounded-lg transition-all active:scale-95`}
        aria-label={label}
      >
        <span className={`${iconColor} text-lg leading-none`}>{icon}</span>
      </button>

      {/* THE CUSTOM POPPING MENU */}
      <div className="absolute top-12 left-1/2 -translate-x-1/2 scale-90 opacity-0 group-hover:opacity-100 group-hover:scale-100 transition-all duration-200 pointer-events-none z-[100] origin-top">
        <div className="bg-[#151515] border border-white/10 p-3 rounded-xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] backdrop-blur-xl w-48">
          {/* Main Title */}
